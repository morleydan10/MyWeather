...
import PatchyRainDay from '../assets/partly-cloudy-day-drizzle.svg';
...
function ForecastConditionIcon({ hour }) {

    function forecastCondition(){

        // Variables to help clean syntax
        const condition = hour.condition.text.toLowerCase();
        const apiIcon = hour.condition.icon
        const day = hour.is_day === 1
        ...
        const patchyRain = condition.includes('light') || condition.includes('patchy') || condition.includes('partly') && condition.includes('rain') || condition.includes('drizzle')
...
export default ForecastConditionIcon;